(function ($) {
    $.fn.jquizzy = function (settings) {
        var defaults = {
            questions: null,
            startImg: '../static/ceshi/images/start.gif',
            endText: '已结束!',
            shortURL: null,
            sendResultsURL: null,
            resultComments: {
        ...
        superContainer.find('.final').click(function () {
            ...
            resultSet = '<h2 class="qTitle">' + judgeSkills(score) + '<br/> 您的分数： ' + score + '<button id="shengji" fenshu="' + score + '">升级为中级管理员?</button>' + '</h2>' + shareButton + '<div' +
                ' class="jquizzy-clear"></div>' + resultSet + '<div class="jquizzy-clear"></div>';
            ...
            $("#shengji").click(function () {
                var fenshu = $("#shengji").attr("fenshu");  //取得分数
                

                if (fenshu >= 99) {
                	$.ajax({
            			type : "POST",
            			url : "/admin/system/user/shengji",
            			dataType : "json",
