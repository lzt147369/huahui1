...
public class MysqlGenerator {
	...
	private static String authorName = "shen-ke"; // 作者
	private static String[] table = {"w_flowers"}; // table名字
	...
	public static void main(String[] args) {
		...
		// .setXml("mapper")
			 ...
			 * .setCfg( // 注入自定义配置，可以在 VM 中使用 cfg.comadmin 设置的值 new
			 * InjectionConfig() {
			 * 
			 * @Override public void initMap() { Map<String, Object> map = new
			 * HashMap<>(); map.put("comadmin",
			 * this.getConfig().getGlobalConfig().getAuthor() + "-mp");
			 * this.setMap(map); }
			 * }.setFileOutConfigList(Collections.<FileOutConfig>singletonList(
			 * new FileOutConfig("/templates/mapper.xml.vm") { // 自定义输出文件目录
			 * 
			 * @Override public String outputFile(TableInfo tableInfo) { return
			 * path+"/src/main/resources/mapper/" + packageName + "/" +
			 * tableInfo.getEntityName() + "Mapper.xml"; } })) ).setTemplate( //
			 * 关闭默认 xml 生成，调整生成 至 根目录 new TemplateConfig().setXml(null) //
			 * 自定义模板配置，模板可以参考源码 /mybatis-plus/src/main/resources/template 使用
			 * copy // 至您项目 src/main/resources/template 目录下，模板名称也可自定义如下配置： //
			 * .setController("..."); // .setEntity("..."); //
			 * .setMapper("..."); // .setXml("..."); // .setService("..."); //
			 * .setServiceImpl("..."); )
			 */;
